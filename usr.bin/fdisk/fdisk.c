...
#ifdef __minix
#define MINIX_SUBPARTITIONS
#ifndef _PATH_DEFDISK
#define _PATH_DEFDISK	"/dev/c0d0"
#endif
#endif
...
#endif
#define OPTIONS	BOOTSEL_OPTIONS MINIX_OPTIONS "0123FSafiIluvA:b:c:E:r:s:w:"
...
#define ROOT		"/"
  ...
  if (stat(ROOT, &rootstat) == 0 && (dp = opendir(DEV_PATH)) != (DIR *) NULL) {
	dev = rootstat.st_dev;
	if (major(dev) == 1) {
/* XXX get sysenv ramimagewhatever for dev/ram ? */
		dev = 0x0300;
	} else if (major(dev) == 3 ||
	          (major(dev) >= 8 && major(dev) <= 14 && ! major(dev) & 1) ) {
		if (minor(dev) < 8*5)
			dev = makedev(major(dev), minor(dev) / 5);
		else if (minor(dev) >= 128 && minor(dev) < 255)
			/* Could be the MINIX surrounding partition too... */
			dev = makedev(major(dev), ((minor(dev)&0x70) >> 4) * 5);
	}
	while ((entry = readdir(dp)) != (struct dirent *) NULL) {
		...
		if (filestat.st_rdev != dev) continue;
		*diskp = namebuf;
		return;
	}
  }
...
usage(void)
{
	...
	(void)fprintf(stderr,
		"usage: %s [-a" BOOTSEL_OPTIONS "FfIil" MINIX_OPTIONS "Suv] "
		"[-A ptn_alignment[/ptn_0_offset]] \\\n"
		"%*s[-b cylinders/heads/sectors] \\\n"
		"%*s[-0123 | -E num "
		"[-s id/start/size[/bootmenu]]] \\\n"
#ifndef __minix_XXX /* no disktab on MINIX */
		"%.*s"
#else
...
#ifdef __minix
	/* XXX 32-bit overflow warning ahead */
	newofs = mul64u(sector, 512);
	if (lseek64(*rfd, newofs, 0, NULL) == -1)
		return (-1);
#else
...
write_disk(daddr_t sector, void *buf)
{
	...
	/* XXX 32-bit overflow warning ahead */
	newofs = mul64u(sector, 512);
	if (lseek64(wfd, newofs, 0, NULL) == -1)
		return (-1);
#else
...
#ifdef __minix
static int
minix_partition2disklabel(int fd, struct disklabel *dlp)
{
...
#ifdef DIOCGETP
	if (r = ioctl(fd, DIOCGETP, &partition) < 0)
		return (r);
	/* XXX Warning: no overflow check (>2TB) */
	dlp->d_secperunit = div64(partition.size, dlp->d_secsize=512);
#ifdef MINIX_SUBPARTITIONS
	if (m_flag)
	    lowsec_offset = div64(partition.base, 512);
#endif
	dlp->d_ncylinders = partition.cylinders;
	dlp->d_ntracks = partition.heads;
	dlp->d_nsectors = partition.sectors;
	return 0;
#else
