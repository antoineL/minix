 ...
 *
 *    m_type      TTY_LINE  USER_ENDPT  COUNT   TTY_PGRP  IO_GRANT
 * -----------------------------------------------------------------
 * | HARD_INT    |         |         |         |         |         |
 * |-------------+---------+---------+---------+---------+---------|
 * | SYS_SIG     | sig set |         |         |         |         |
 * |-------------+---------+---------+---------+---------+---------|
 * | DEV_READ    |minor dev| proc nr |  count  |  pgid   |  grant  |
 * |-------------+---------+---------+---------+---------+---------|
 * | DEV_WRITE   |minor dev| proc nr |  count  |  pgid   |  grant  |
 * |-------------+---------+---------+---------+---------+---------|
 * | DEV_IOCTL   |minor dev| proc nr |func code|  pgid   |  grant  |
 * |-------------+---------+---------+---------+---------+---------|
 * | DEV_OPEN    |minor dev| proc nr | O_NOCTTY|   sid   |         |
 * |-------------+---------+---------+---------+---------+---------|
  ...
  switch (m_ptr->TTY_REQUEST) {
	...
	if (tp->tty_proc == 0)
		...
		r = sys_safecopyfrom(m_ptr->m_source,
			...
			(vir_bytes) &param.pgid, (vir_bytes) size);
	if (r != OK) break;
	if (param.pgid <= 0)
		r = EINVAL;
#if !defined(_POSIX_JOB_CONTROL) || _POSIX_JOB_CONTROL<=0
	else if (tp->tty_fg_pgid != param.pgid)
	/* If job control is not supported, disallow change of the group */
		r = EINVAL;
#endif
	else
		tp->tty_fg_pgid = param.pgid;
	break;
	...
	if (tp->tty_proc == 0 || tp->tty_fg_pgid == 0)
		...
		r = sys_safecopyto(m_ptr->m_source,
			(cp_grant_id_t) m_ptr->IO_GRANT, 0,
			(vir_bytes) &tp->tty_fg_pgid, (vir_bytes) size);
	break;

    case TIOCGSID:
	/* This ioctl should have been intercepted by VFS... */
	r = ENOTTY;
	break;
