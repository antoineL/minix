...
#endif
#ifdef MINIX_SUBPARTITIONS
#define MINIX_OPTIONS	"m"
#else
#define MINIX_OPTIONS	
#endif
#define OPTIONS	BOOTSEL_OPTIONS "0123FSafiIluvA:b:c:E:r:s:w:z:" MINIX_OPTIONS
...
#ifdef MINIX_SUBPARTITIONS
/* MINIX since 1.6 uses a hack to increase the numbers of available
 * partitions while using the original MBR scheme, without the potential
 * complexity of the "extended" and "logical" partitions. Each partition
 * (named cXdYpZ) has itself a partition table at offset 446 of sector 0
 * (like the MBR), slicing the space into up to four sub-partitions.
 *
 * Invoking fdisk against cXpYdZ allows to edit the sub-partitions,
 * but the code has to compensate for the first sector to be non-0.
 *
 * Alignment constraint default should be relaxed to 1/1.
 */
static int m_flag;		/* MINIX sub-partition mode */
static unsigned lowsec_offset;	/* Offset. */
#endif /* MINIX_SUBPARTITIONS */
...
#define	MOUNTS_PATH	"/proc/mounts"
	...
	FILE *mtab;
	int n;
	static char mount[NAME_MAX + 8];

	if( (mtab = fopen(MOUNTS_PATH, "r")) == NULL)
		return;
	while( fgets(mount, sizeof mount, mtab) != NULL) {
		if( sscanf(mount, "%*s on / %n", &n) == 1) {
			/* found the line for the root device */
			strtok(mount, " ");
			*diskp = mount;
			break;
		}
...
main(int argc, char *argv[])
{
	...
	while ((ch = getopt(argc, argv, OPTIONS)) != -1) {
		...
		case 'w':	/* write data to disk_file */
			...
			break;
		case 't':
#ifdef __minix /* no disktab on MINIX */
			warnx("-t disktab has no effect on MINIX");
#else
			if (setdisktab(optarg) == -1)
				errx(EXIT_FAILURE, "bad disktab");
#endif
			break;
/* XXX TODO: improve the documentation about -T and its behaviour on MINIX */
		case 'T':
			disk_type = optarg;
			break;
#ifdef MINIX_SUBPARTITIONS
		case 'm':	/* MINIX subpartitions. */
			m_flag = 1;
			break;
#endif
		case 'z':
...
/* XXX check disk_type validity... */
...
#ifdef MINIX_SUBPARTITIONS
	if (m_flag && lowsec_offset == 0)
		warnx("MINIX subpartitions (-m) but offset unknown?");
#endif
	...
	if (ptn_alignment == 0)
#ifdef MINIX_SUBPARTITIONS
/* XXX Deal with the 32-sector hole for bootxx... */
	  if (m_flag)
		ptn_alignment = ptn_0_offset = 1;
	  else
#endif
		get_ptn_alignmemt();
...
usage(void)
{
	...
	(void)fprintf(stderr, "usage: %s [-aBFfIil" MINIX_OPTIONS "Suv] "
...
#ifdef MINIX_SUBPARTITIONS
		"\t-m act on MINIX subpartitions (c#d#p#)\n"
#endif
...
#ifdef __minix
	/* XXX 32/64-bit compatibility stuff ahead, to be removed later */
	big_off_t offs = sector * (big_off_t)512;
	big_off_t mod = offs & (secsize - 1);
	big_off_t rnd = offs & ~(secsize - 1);

	if (lseek64(*rfd, rnd, SEEK_SET, NULL) == -1)
		return -1;
#else
...
#ifdef __minix
	/* XXX again 32/64-bit compatibility stuff, to be removed */
	big_off_t offs = sector * (big_off_t)512;
	big_off_t mod = offs & (secsize - 1);
	big_off_t rnd = offs & ~(secsize - 1);
...
#ifdef DIOCGETP
	r = ioctl(dfd, DIOCGETP, &part);
	if (r != 0)
		return r;
	if (part.size > UINT32_MAX * 512)
		dlp->d_secperunit = UINT32_MAX;
	else
		dlp->d_secperunit = part.size / (dlp->d_secsize=512);
#ifdef MINIX_SUBPARTITIONS
	/* XXX Warning: no overflow check (base>2TB) */
	lowsec_offset = m_flag ? part.base / 512 : 0;
#endif
	dlp->d_ncylinders = part.cylinders;
	dlp->d_ntracks = part.heads;
	dlp->d_nsectors = part.sectors;
	return 0;
#else /* no DIOCGETP */
	#error Cannot guess geometry of disk_type
#endif
}
#else /* ! MINIX */
/* XXX in cross-compilation cases, we need some tool too... */
#endif
...
/* XXX Actually useful? Drop all that stuff? */

		int disktype_fd;

		if ( (disktype_fd=open(disk_type, O_RDONLY)) < 0) {
			warn("open %s", disk_type);
			return (-1);
		}
		if (minix_partition2disklabel(disktype_fd, &disklabel) < 0) {
			warn("DIOCGETP on %s", disk_type);
			close(disktype_fd);
			return (-1);
		}
		close(disktype_fd);
	...
	} else if (minix_partition2disklabel(fd, &disklabel) != 0) {
		warn("DIOCGETP");
		return(-1);
#else
	} else {
		warnx("no DIOCGDEFLABEL"); /* consider -F or -b ? */
		return (-1);
#endif
	}
...
write_mbr(void)
{
	...
	rval = 0;
#ifdef __minix
/* XXX notify the running system, using DIOCSETP... */
#endif
    protect_label:
	flag = 0;
#ifdef DIOCWLABEL
