         ...
         int blocksize, dev_t majordev)
{
  ...
  if(env_get_param("constrained", bla, sizeof(bla)) == OK)
	  return minbufs;
...
lmfs_alloc_block(struct buf *bp)
{
  ...
  if((bp->data = minix_mmap(0, fs_block_size,
     PROT_READ|PROT_WRITE, MAP_PREALLOC|MAP_ANON, -1, 0)) == MAP_FAILED)
  	panic("fs cache: couldn't allocate a new block");
  assert(bp->data);
 ...
 */
  ...
  if((ino != VMC_NO_INODE && ino_off > 2000000000)) {
  	printf("cache: suspiciously big ino offset %llu\n", ino_off);
  	util_stacktrace();
  }
...
#if 0
		ASSERT(bp->cache_dev != NO_DEV);
		ASSERT(bp->cache_dev == dev);
		ASSERT(bp->cache_off == dev_off);
#endif
...
#if 0
	printf("libminixfs: vm_map_cacheblock of dev 0x%x offset 0x%llx failed\n",
		dev, dev_off);
	util_stacktrace();
#endif
  }
