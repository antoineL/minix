...
#define GETDENTS_BUFSIZ  (256+sizeof(struct dirent))
#endif
...
/* NOTE: can fail with EOF */
	...
	if (block_pos < pos)
#if 1
		fatdp = &bp->b_dir[off / DIR_ENTRY_SIZE];
#elif 0
...
#if 1
	for (; fatdp < &bp->b_dir[block_size / DIR_ENTRY_SIZE]; fatdp++) {
#elif 0
	...
	for (; fatdp < (struct fat_direntry *) &bp->b_dir[16]; fatdp++) {
		...
		if (fatdp->d_direntry.deName[0] == SLOT_EMPTY) {
			done = TRUE;
#if 0
FIXME: use done
			return(ENOENT);
		...
		/* is this slot in use */
		else if (fatdp->d_direntry.deName[0] == SLOT_DELETED) {
			continue;
		}
		/* is this entry for long names? */
		else if (fatdp->d_lfnentry.lfnAttributes == ATTR_LFN) {
			continue;
		}
#if 0
...
/* FIXME: should rather be (char *)bp->b_dir, methinks */
		ent_pos = block_pos + ((char *) dep - (bp->b_data));
...
/* NOTE: can fail with EOF */
	...
	/* Search a directory block. */
	for (dep = &bp->b_dir[0];
			dep < &bp->b_dir[block_size / DIR_ENTRY_SIZE];
			dep++) {

		/* is the EndOfDirectory mark found */
		if (dep->d_direntry.deName[0] == SLOT_EMPTY) {
			return(ENOENT);
		}
		/* is this slot in use */
		else if (dep->d_direntry.deName[0] == SLOT_DELETED) {
			continue;
		}
		/* is this entry for long names? */
		else if (dep->d_lfnentry.lfnAttributes == ATTR_LFN) {
			continue;
		}
	...
	put_block(bp);		/* continue searching dir */
  }
