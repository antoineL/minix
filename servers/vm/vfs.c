...
#if 0
	printf("VM to VFS: sent request %d with target ep %d\n",
		active->reqmsg.VFS_VMCALL_REQ,
		active->reqmsg.VFS_VMCALL_ENDPOINT);
#endif
...
#if 0
	printf("VM: asynsend for ep %d reqid %d done\n",
		active->who, active->req_id);
#endif
}
 ...
 */
	...
	if(vm_isokendpt(m->VMV_ENDPOINT, &n) != OK)
		panic("VM: received VFS reply for bad ep %d", m->VMV_ENDPOINT);
	vmp = &vmproc[n];
