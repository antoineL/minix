	...
	int protflags)
{
...
#if 0
	printf("vfs_memmap: calling minix_vfs_mmap(%d, offset 0x%lx, len 0x%lx, dev 0x%x, inode %d, fd %d, vaddr 0x%lx, clearend 0x%x)\n",
		execi->proc_e, foffset, len, vp->v_dev, vp->v_inode_nr, vi->procfd, vaddr, clearend);
#endif
	...
	if(r == OK) {
#if 0
		printf("vfs_memmap: OK\n");
#endif
		vi->procfd_used = 1;
	}
#if 0
	else printf("vfs_memmap: not OK, %d\n", r);
#endif
...
#if 0
			printf("exec open %s for mmap: %d\n", firstexec, execi.procfd);
#endif
