...
int do_get()
{
/* Handle GETUID, GETGID, GETGROUPS, MINIX_GETPID, GETPGRP, GETPGID_SID,
   ISSETUGID.
 */
...
int do_set()
{
/* Handle SETUID, SETEUID, SETGID, SETGROUPS, SETEGID, and SETSID, SETPGID.
 * These calls have in common that, if successful, they will be forwarded to
 * VFS as well.
 */
  register struct mproc *rmp = mp;
  struct mproc *emp;
