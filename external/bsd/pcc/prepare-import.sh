...
# fix existing RCS and NetBSD tags
...
mkdir work/tmp
cd work/tmp
env -i PATH=/bin:/usr/bin /bin/sh ../pcc/configure --disable-striping --prefix=/usr # --enable-tls
cd ../..
#
...
echo "    git add -a dist"
echo '    git commit --author="Anders Magnusson <ragge@ludd.ltu.se>"'
echo ""
